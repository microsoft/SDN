...
while ((Get-Job -Id $($thisJob).Id).state -eq "Running") {
    try {
        # WS2025 - attempt to get the counter for "SMB Direct"
        $written = Get-Counter -Counter "\\$RemoteIpAddress\SMB Direct\Bytes RDMA Written/sec" -ErrorAction Stop
        $sent = Get-Counter -Counter "\SMB Direct\Bytes Sent/sec" -ErrorAction Stop
    if ($written -ne $null) {
    ...
    if ($sent -ne $null) {
        ...
        try {
            # WS2022 - attempt to get the counter for "SMB Direct Connection"
            $written = Get-Counter -Counter "\\$RemoteIpAddress\SMB Direct Connection(_Total)\Bytes RDMA Written/sec" -ErrorAction Stop
            $sent = Get-Counter -Counter "\SMB Direct Connection(_Total)\Bytes Sent/sec" -ErrorAction Stop
    if ($written -ne $null) {
        $RdmaWriteBytesPerSecond = [uint64]($written.Readings.split(":")[1])
        if ($RdmaWriteBytesPerSecond -gt 0) {
            $RdmaTrafficDetected = $true
    ...
    if ($sent -ne $null) {
        ...
        if ($OutputLevel -gt 0) {
            ...
            Write-Host "SMB Direct Connection counters not found."
        }
    }
}
