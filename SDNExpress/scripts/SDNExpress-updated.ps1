            ...
            foreach ($VMInfo in $GatewayVMList) {
                Script "SetPort_$($VMInfo.VMName)"
                {
                    SetScript = {
                        ...
                        set-portprofileid -ResourceID $InternalNicInstanceid -vmname $using:VMInfo.VMName -VMNetworkAdapterName "Internal" -computername $node.NodeName -ProfileData "1" -Force
                        write-verbose ("VM - $($using:VMInfo.VMName), Adapter - External")
                        set-portprofileid -ResourceID $ExternalNicInstanceid -vmname $using:VMInfo.VMName -VMNetworkAdapterName "External" -computername $node.NodeName -ProfileData "1" -Force
                    }
                    TestScript = {
