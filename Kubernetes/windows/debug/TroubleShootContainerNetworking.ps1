    ...
    [void] PrepareReport() {
        ...
        foreach ($diagnosticTest in $this.DiagnosticTests) {
            ...
            if ($status -eq [TestStatus]::Failed) {
                ...
                $problemsDetected += [PSCustomObject]$testReport
            }
            else {
                $test = @{
                ...
                $issuesChecked += [PSCustomObject]$test
            }
...
"@
        ...
        $problemsDetectedHtml = $problemsDetected | Select-Object Problem, Resolution | ConvertTo-Html -Fragment -PreContent "<h2 style='color: red'>Problems Detected</h2>"
        $issuesCheckedHtml = $issuesChecked | Select-Object Test, Status, Comments | ConvertTo-Html -Fragment -PreContent "<h2 style='color: green';>Potential Issues Checked</h2>"
...
if (-NOT $Replay.IsPresent) {
    Copy-Item ..\TroubleShootContainerNetworking.ps1 .
